...
private:
	void ProcessBundle(const char* data, unsigned int len, WOscNetReturn* networkReturnAddress);
	void ProcessMessage(const char* data, unsigned int len, const WOscNetReturn* networkReturnAddress);
	...
	/** The callback list of methods which match a certain message. The object
	 * is cached here to avoid reallocation for each message dispatch. The size
	 * of the vector is kept. Reallocation happens only if capacity is low.
	 */
	WOscCallbackList	m_callbackList;
