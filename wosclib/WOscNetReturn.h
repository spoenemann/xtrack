...
protected:
	...
	/** The parent object (WOscQueueItem) determines if a network-return
	 * address is an orphan by adding and removing itself as a parent. */
	friend class WOscQueueItem;
	/** When receiving a message, the network-return-address has to be deleted
	 * in receiver. */
...
private:
	WOscNetReturn(const WOscNetReturn&);
	WOscNetReturn& operator=(const WOscNetReturn&);

	/** Number of WOscQueueItems are using this element currently.*/
	int  m_numUsers;
};
