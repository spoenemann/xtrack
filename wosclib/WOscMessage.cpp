...
WOscMessage::WOscMessage(const char* address){
	...
	if ( address == NULL )
		throw WOscException(ERR_NULL_ADDRESS, __FILE__, __LINE__);
...
WOscMessage::WOscMessage(const char* buffer, int bufferLen){
	...
	// check address
	if (bufferLen < 1)
		throw WOscException(ERR_NULL_BUFFER, __FILE__, __LINE__);
	// check address
	if (buffer[0] != '/')
		throw WOscException(ERR_INVALID_ADDRESS_NO_SLASH, __FILE__, __LINE__);
	...
	for ( int i = 0; i < typeTagSize; i++ ){
		switch (typeTag[i]){
			case 'i':
				Add(WOscUtil::BufferTo<int>(buffer + pos));
				pos += WOscUtil::OSC_INT_SIZE;
				break;
			case 'f':
				Add(WOscUtil::BufferTo<float>(buffer + pos));
...
void WOscMessage::GenerateBufferFromMembers(){
	...
	for ( i = 0; i < typeTagSize; i++ ){
		switch (typeTag[i]){
			case 'i': // integer args
				/* write arg to buffer */
				WOscUtil::FillBufferWith(&(m_buffer[bufWrPos]), m_ints[idx_i]);
				...
				break;
			case 'f': // float args
				WOscUtil::FillBufferWith(&(m_buffer[bufWrPos]), m_floats[idx_f]);
 ...
 */
float
WOscMessage::GetFloat(int idx) const
{
	if ( idx > m_numFloats )
 ...
 */
int
WOscMessage::GetInt(int idx) const
{
	if ( idx > m_numInts )
		throw WOscException(ERR_INVALID_INDEX, __FILE__, __LINE__);
	return m_ints[idx];
 ...
 */
const WOscBlob*
WOscMessage::GetBlob(int idx) const
{
	if ( idx > m_numBlobs )
		throw WOscException(ERR_INVALID_INDEX, __FILE__, __LINE__);
	return m_blobs[idx];
...
WOscMessage::GetBuffer(char* buffer, int bufferLen)
{
	if ( bufferLen < m_bufferLen )
		throw WOscException(ERR_OSC_STR_BUFF_TOO_SMALL, __FILE__, __LINE__);
