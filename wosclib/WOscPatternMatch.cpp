...
/** Allowing wildcards in address names (containers and methods). Supported
 * wildcards are "*" and "?". The behaviour is just tested basically in
 * tests/patternmatch.cpp but will require much more testing. If anyone
 * has a nice test please add it to this testbench or notify me.
 *
 * Currently the address space wildcards are on by default.
 */
#ifndef WOSC_USE_ADDR_WILDCARDS
#	define WOSC_USE_ADDR_WILDCARDS 1
#endif
 ...
 *
 * \param pattern
 * The pattern against test has to match. Usually the OSC message address.
 * 
 * \param test
 * The string to be tested. Usually the container address.
 * 
 * \returns
 ...
 */
bool
WOscPatternMatch::PatternMatch (const char *  pattern, const char * test)
{

	if (pattern == 0 || pattern[0] == 0) {
#if WOSC_USE_ADDR_WILDCARDS
		switch (test[0]) {
		case '*': return PatternMatch(pattern, test+1);
		case 0: return true;
	...
	// test has precedence before pattern
	switch ( test[0] ) {
	case 0:
		if (pattern[0] == '*')
...
#if WOSC_USE_ADDR_WILDCARDS
	case '*':
		if (PatternMatch (pattern, test+1)) {
			return true;
		} else {
			return PatternMatch (pattern+1, test);
		}
	...
	// test pattern (message address) against test (container address)
	switch (pattern[0]) {
	case 0      : return test[0] == 0; // already tested at the beginning
	case '?'    : return PatternMatch (pattern + 1, test + 1);
	case '*'    : 
...
bool
WOscPatternMatch::MatchBrackets (const char *pattern, const char *test)
{
	bool result;
	bool negated = false;
 ...
 */
bool
WOscPatternMatch::MatchList (const char *pattern, const char *test)
{
 ...
 */
const char*
WOscPatternMatch::NextSlashOrNull(const char *p)
{
	while (*p != '/' && *p != '\0') {
		p++;
	}
